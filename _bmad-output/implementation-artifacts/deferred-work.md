...
## Deferred from: review of spec-questions-import-ui-redesign (2026-07-17)
...
## Deferred from: one-shot fix-types-sepay-payment-fields (2026-07-20)

- Refine `CheckoutResult` / `PaymentDetail` VietQR fields into a discriminated union on `checkoutMode` (avoid `?: string | null` triple-state).
- Extract shared `VietQrPaymentFields`; align API `CreateCheckoutResult` nullability with `@practice-exam/types`.
- Scope bank account fields off public candidate DTOs or add masking; keep plaintext bank details admin-only where possible.
